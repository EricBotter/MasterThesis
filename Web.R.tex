...
A blockchain is an ordered sequence of \emph{blocks}. Each block contains some data, which are only transactions in the case of Bitcoin, but they also include operations and records for Namecoin. %todo - not correct, Namecoin seems to use the operations of Bitcoin Script
...
Namecoin also defines \emph{records}, which are key-value pairs, and \emph{operations}, which define the actions that are to be applied to a record. %todo - simplify this sentence
