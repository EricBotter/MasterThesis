...
The model in which the current Web is set based on a stop failure model. This model allows Web clients (browsers) and content creators to make certain assumptions on the behavior of web servers and other components:
\begin{itemize}
...
In Table~\ref{table:dwebcomparison} we summarize the main features and characteristics that ZeroNet and Blockstack exhibit. From the table it looks clear how ZeroNet does not reach our requirements to be a decentralized Web, while Blockstack is more fit but still lacks some properties, especially considering how the current system is used for user data only, instead of also storing websites.
