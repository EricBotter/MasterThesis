...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|l|l|l|} \hline
			...
			File container & Torrents & IPFS Objects \\ \hline
			
			Directory support & Yes & Yes \\ \hline

			Link support & No & Yes \\ \hline
...
The public resolver contract deployed on the Ethereum mainnet (\cite{enspublicresolver}) also supports \texttt{content} records, used to handle content-based hashes like the ones found in Ethereum Swarm, \texttt{pubkey} records, which store elliptic-curve cryptography public keys, and \texttt{text} records, which simply store some textual data.
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|p{4.5cm}|p{3cm}|p{4.5cm}|} \hline
			...
			Name registration procedure & Two-step claim & Vickrey blind auction
			\\ \hline
			
			Expected time of name reg. & Two hours & Five days
			\\ \hline
...
%		\begin{tabular}{|p{3.5cm}|p{4.5cm}|p{4cm}|} \hline
			\textbf{Feature} & \textbf{ZeroNet} & \textbf{Blockstack} & \textbf{Tor} \\ \hline
			
			Storage system & Custom & Gaia storage system & Web based \\ \hline
			
			Storage discovery & BitTorrent trackers & Atlas peer network & DHT \\ \hline
			
			Naming system & Namecoin & Blockchain Name System & \emph{Public-key based} \\ \hline
...
%			\end{tabular}	\\ \hline
			...
			Censorship resistance & No (uses trackers) & No (centralized storage) & Yes \\ \hline
			
			Lack of trust & No (uses trackers) & Yes & Yes \\ \hline
			
			Privacy controls & No & Yes & No \\ \hline
		\end{tabular}
	\end{center}
	\caption{Summary of features and characteristics of the presented distributed Web projects. Tor is included for comparison with the described platforms. Note that Tor's naming system does not produce human-meaningful names and thus does not challenge Zooko's Triangle.}
